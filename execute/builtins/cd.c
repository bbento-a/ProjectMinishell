...
/*   Updated: 2025/01/27 11:15:35 by mde-maga         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int go_to_path(int option, t_env *env)
{
    ...
    if (option == 0) // Go to HOME
    {
        if (!(env_path = get_env_path(env, "HOME=", 5)))
            return (ft_putendl_fd("minishell: cd: HOME not set", STDERR), ERROR);
    }
    else if (option == 1) // Go to OLDPWD
    {
        if (!(env_path = get_env_path(env, "OLDPWD=", 7)))
            return (ft_putendl_fd("minishell: cd: OLDPWD not set", STDERR), ERROR);
